...
def load_dataframe(x, extension=None, debug=False, **kwargs):
    ...
    :param extension: optional argument for specifying the filetype (e.g., 'csv', 'xls', etc.).  If no extension is
    specified,
    ...
    :param debug: internal flag used for debugging (prints a warning message if filetype cannot be inferred
    automatically);
      default: False.
    :param kwargs: any additional keyword arguments are passed directly to the relevant Pandas function for reading
    in the
      inferred data type.
